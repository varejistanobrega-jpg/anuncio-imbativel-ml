...
export default async function handler(req, res) {
  try {
    ...
    /*
     * 3. Consulta o anúncio primeiro para confirmar
    ...
    /*
     * 5. Consulta exclusivamente a estrutura de entrada
     * da ficha técnica atual da categoria.
     */
    const technicalSpecsResult = await getResource(
      `https://api.mercadolibre.com/categories/${encodeURIComponent(categoryId)}/technical_specs/input`,
      accessToken
    );

    if (!technicalSpecsResult.ok) {
      return res.status(200).json({
        ok: true,
        resource_status: "unavailable",
        resource_http_status: technicalSpecsResult.status,
        item_id: itemId,
    ...
    console.error(
      "Erro ao consultar ficha técnica da categoria:",
    ...
    return res.status(500).json({
      ok: false,
      error:
        "Erro interno ao consultar ficha técnica da categoria"
    });
  }
}
